// PRU code to test shared memory contents: sweep through shared memory
// and write to sense pins
#include <stdint.h>
#include <pru_cfg.h>
...
void main(void)
{
	...
	while (1) {
	  for (iface->lastaddr = 0; iface->lastaddr < 36 * 1024; iface->lastaddr++) {
	    iface->lastdata = shared[iface->lastaddr];
	    gpio2[GPIO_DATAOUT] = iface->lastdata << 1;
	  }
	}
}
